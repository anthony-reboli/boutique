...
// categoerie
		...
		//echo"</section>";
					 ...
					 // if (isset($_GET['page'])== $i and $_GET['c'] == 'the')
					 // {

					 // 		$pagetotal= ceil($resu[0][0]/$arpage);
						// 	for ($i=1;$i <=$pagetotal;$i++)
						// 	{
						// 	echo '<a href="boutique.php?page='.$i.'& c='.$_POST['pagear'].'">'.$i.'</a> ';
						// 	}

						// 	$requetee='SELECT * FROM produits where categories = 1 ORDER BY id DESC limit '.$depart.','.$arpage.' ';
					
						// 	$queryy3=mysqli_query($bdd, $requetee);
						// 	echo "rentre";
